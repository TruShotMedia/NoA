...
const NOA_SESSION_COOKIE = 'noa_session';
const NOA_IDENTITY_COOKIE = 'noa_identity';
const NOA_SESSION_TTL_SECONDS = 5 * 60;
const NOA_IDENTITY_TTL_SECONDS = 30 * 24 * 60 * 60;
const NOA_AUTH_ATTEMPT_WINDOW_MS = 10 * 60 * 1000;
const NOA_AUTH_MAX_ATTEMPTS = 6;
const noaAuthAttempts = new Map();
const BUDGET_TABLES = {
...
const BUDGET_DEFAULT_CATEGORY_COLORS = ['#7dd3fc', '#a78bfa', '#34d399', '#fbbf24', '#fb7185', '#60a5fa', '#f472b6', '#c084fc', '#94a3b8'];
const NOA_GROCERY_LIST_PERSONALISATION_KEY = 'grocery_list_personalisation';
const NOA_SCREENSAVER_BUCKET = 'noa-screensavers';
const NOA_JSON_WARN_BYTES = 512 * 1024;
const BUDGET_TABLE_SELECTS = {
  ledger_income: 'id,user_id,mode,local_id,name,amount,frequency,tax_rate,active,schedule_type,schedule_day,schedule_date,schedule_exact_date,notes,weekly_amount,monthly_amount,raw_data,updated_at',
  ledger_expenses: 'id,user_id,mode,local_id,name,category,amount,frequency,active,schedule_type,schedule_day,schedule_date,schedule_exact_date,notes,weekly_amount,monthly_amount,raw_data,updated_at',
  ledger_debts: 'id,user_id,mode,local_id,name,debt_type,balance,repayment,frequency,interest_rate,active,schedule_type,schedule_day,schedule_date,schedule_exact_date,notes,weekly_repayment,monthly_repayment,raw_data,updated_at',
  ledger_mortgages: 'id,user_id,mode,local_id,name,property_address,balance,property_value,repayment,frequency,interest_rate,active,schedule_type,schedule_day,schedule_date,schedule_exact_date,notes,weekly_repayment,monthly_repayment,raw_data,updated_at,tenant_count',
  ledger_mortgage_expenses: 'id,user_id,mode,mortgage_local_id,local_id,name,amount,frequency,active,schedule_type,schedule_day,schedule_date,schedule_exact_date,notes,weekly_amount,monthly_amount,raw_data,updated_at,offset_to_tenants,tenant_bill_weekly,tenant_bill_monthly,tenant_bill_per_tenant_weekly,tenant_bill_per_tenant_monthly',
  ledger_assets: 'id,user_id,mode,local_id,name,asset_type,value,active,notes,raw_data,updated_at',
  ledger_savings: 'id,user_id,mode,amount,frequency,goal_name,goal_amount,active,weekly_amount,monthly_amount,raw_data,updated_at'
};
const GROCERY_ITEM_SELECT = 'id,item,quantity,category,added_by,added_by_user_id,completed,completed_at,created_at,updated_at';
let spotifyAccessTokenCache = { accessToken: '', expiresAt: 0 };
...
function sendJson(res, status, body) {
  ...
  res.setHeader('x-noa-json-bytes', String(Buffer.byteLength(json, 'utf8')));
  if (json.length > NOA_JSON_WARN_BYTES || /data:image\/(?:jpeg|jpg|png|webp);base64,/i.test(json)) {
    console.warn(`NoA large JSON response: ${Buffer.byteLength(json, 'utf8')} bytes, containsDataImage=${/data:image\/(?:jpeg|jpg|png|webp);base64,/i.test(json)}`);
  }
  res.end(json);
}
...
async function handleNoaAuth(req, res) {
  ...
  if (action === 'logout') {
    clearNoaSessionCookie(req, res);
    clearNoaIdentityCookie(req, res);
    return {
      ok: true,
      authenticated: false,
      unlocked: false,
      locked: true,
      message: 'NoA signed out.'
    };
  }
  ...
  if (action === 'lock') {
    ...
    return {
      ok: true,
      authenticated: Boolean(verifyNoaIdentityCookie(req)),
      locked: true,
      message: 'NoA locked.'
    };
  }
  ...
  if (action === 'status') {
    const identity = verifyNoaIdentityCookie(req);
    const unlocked = Boolean(identity) && verifyNoaSessionCookie(req);
    return {
      ok: true,
      ...
      unlocked,
      locked: !unlocked,
      expiresIn: unlocked ? getNoaSessionSecondsRemaining(req) : 0
    };
  }
  ...
  if (action === 'login') {
    if (!isTrustedRequestOrigin(req)) {
      ...
      return {
        ok: false,
        status: 'signed-out',
        message: 'NoA rejected this login request because it did not come from this app.'
      };
    }

    const rateLimit = getNoaAuthRateLimit(req);
    ...
    const login = await verifySupabaseNoaLogin(email, password);
    if (!login.ok) {
      recordNoaAuthFailure(req);
      clearNoaIdentityCookie(req, res);
      clearNoaSessionCookie(req, res);
      return {
        ok: false,
    ...
    clearNoaAuthFailures(req);
    setNoaIdentityCookie(req, res, login.email);
    setNoaSessionCookie(req, res);
    return {
      ok: true,
      authenticated: true,
      authEmail: login.email,
      unlocked: true,
      expiresIn: NOA_SESSION_TTL_SECONDS,
      message: 'NoA signed in.'
    };
  }
  ...
  if (!isTrustedRequestOrigin(req)) {
    ...
    return {
      ok: false,
      status: 'locked',
      message: 'NoA rejected this unlock request because it did not come from this app.'
    };
  }
  ...
  if (!identity) {
    ...
    return {
      ...
      authenticated: false,
      status: 'signed-out',
      message: 'Sign in with the authorised Supabase account before using the PIN lock.'
    };
  }
  ...
  if (rateLimit.blocked) {
    ...
    return {
      ...
      status: 'locked',
      retryAfterSeconds: rateLimit.retryAfterSeconds,
      message: `Too many incorrect PIN attempts. Try again in ${rateLimit.retryAfterSeconds} seconds.`
    };
  }
  ...
  return {
    ...
    unlocked: true,
    expiresIn: NOA_SESSION_TTL_SECONDS,
    message: 'NoA unlocked.'
  };
}

function requireNoaAuth(req, res, options = {}) {
  if (req.method === 'OPTIONS') return true;
  if (options.allowCron && isAuthorizedCronRequest(req)) return true;
  ...
  if (!identity) {
    sendJson(res, 401, {
      ...
      authenticated: false,
      locked: true,
      message: 'NoA requires Supabase sign-in before private data can be accessed.'
    });
    return false;
  }

  if (verifyNoaSessionCookie(req)) return requireTrustedMutationOrigin(req, res);
  ...
  sendJson(res, 401, {
    ...
    authEmail: identity.email,
    locked: true,
    message: 'NoA is locked. Unlock the app with your PIN first.'
  });
  return false;
}

function requireTrustedMutationOrigin(req, res) {
  if (!['POST', 'PUT', 'PATCH', 'DELETE'].includes(String(req.method || '').toUpperCase())) return true;
  if (isAuthorizedCronRequest(req)) return true;
  if (isTrustedRequestOrigin(req)) return true;

  sendJson(res, 403, {
    ok: false,
    locked: true,
    message: 'NoA blocked this request because it did not come from this app.'
  });
  return false;
}

function isAuthorizedCronRequest(req) {
  const cronSecret = process.env.CRON_SECRET || process.env.NO_A_CRON_SECRET || '';
...
function getNoaPin() {
  return process.env.NOA_PIN || '8726';
}

function getNoaSessionSecret() {
  return process.env.NOA_SESSION_SECRET
    || process.env.NOA_AUTH_SECRET
...
function verifyNoaPin(pin) {
  const configuredHash = process.env.NOA_PIN_HASH || '';
  if (configuredHash) return verifyNoaPinHash(pin, configuredHash);
  return timingSafeEqual(pin, getNoaPin());
}

function verifyNoaPinHash(pin, configuredHash) {
  const [scheme, iterationsText, salt, expectedHash] = String(configuredHash).split('$');
  const iterations = Number(iterationsText);
  if (scheme !== 'pbkdf2_sha256' || !Number.isInteger(iterations) || iterations < 100000 || !salt || !expectedHash) return false;
  const actualHash = crypto.pbkdf2Sync(String(pin), salt, iterations, 32, 'sha256').toString('base64url');
  return timingSafeEqual(actualHash, expectedHash);
}

function setNoaSessionCookie(req, res) {
  const expiresAt = Date.now() + NOA_SESSION_TTL_SECONDS * 1000;
  const token = signNoaSession(expiresAt);
  appendSetCookie(res, serializeCookie(NOA_SESSION_COOKIE, token, req, {
    httpOnly: true,
    maxAge: NOA_SESSION_TTL_SECONDS,
    sameSite: 'Lax',
    path: '/'
  }));
}

function clearNoaSessionCookie(req, res) {
  appendSetCookie(res, serializeCookie(NOA_SESSION_COOKIE, '', req, {
    httpOnly: true,
    maxAge: 0,
    sameSite: 'Lax',
    path: '/'
...
function setNoaIdentityCookie(req, res, email) {
  const expiresAt = Date.now() + NOA_IDENTITY_TTL_SECONDS * 1000;
  const payload = Buffer.from(JSON.stringify({ email: String(email || '').toLowerCase(), expiresAt })).toString('base64url');
  const signature = createNoaSessionSignature(`identity:${payload}`);
  appendSetCookie(res, serializeCookie(NOA_IDENTITY_COOKIE, `${payload}.${signature}`, req, {
    httpOnly: true,
    maxAge: NOA_IDENTITY_TTL_SECONDS,
    sameSite: 'Lax',
    path: '/'
  }));
}

function clearNoaIdentityCookie(req, res) {
  appendSetCookie(res, serializeCookie(NOA_IDENTITY_COOKIE, '', req, {
    httpOnly: true,
    maxAge: 0,
    sameSite: 'Lax',
    path: '/'
...
function verifyNoaIdentityCookie(req) {
  const token = parseCookies(req.headers.cookie || '')[NOA_IDENTITY_COOKIE];
  if (!token) return null;
  const [payload, signature] = token.split('.');
  if (!payload || !signature || !timingSafeEqual(signature, createNoaSessionSignature(`identity:${payload}`))) return null;
  try {
    ...
    if (!isNoaAuthEmailAllowed(email)) return null;
...
function verifyNoaSessionCookie(req) {
  const token = parseCookies(req.headers.cookie || '')[NOA_SESSION_COOKIE];
  if (!token) return false;
  const [expiresAtText, signature] = token.split('.');
  const expiresAt = Number(expiresAtText);
  if (!Number.isFinite(expiresAt) || expiresAt < Date.now() || !signature) return false;
  return timingSafeEqual(signature, createNoaSessionSignature(expiresAtText));
}

function getNoaSessionSecondsRemaining(req) {
  const token = parseCookies(req.headers.cookie || '')[NOA_SESSION_COOKIE];
  if (!token) return 0;
...
function signNoaSession(expiresAt) {
  return `${expiresAt}.${createNoaSessionSignature(String(expiresAt))}`;
}

function createNoaSessionSignature(value) {
  return crypto
    .createHmac('sha256', getNoaSessionSecret())
    .update(String(value))
    .digest('base64url');
}

async function verifySupabaseNoaLogin(email, password) {
  if (!email || !password) {
    return { ok: false, message: 'Enter your Supabase email and password.' };
  }
  if (!isNoaAuthEmailAllowed(email)) {
    return { ok: false, message: `${email} is not authorised for NoA.` };
  }
  ...
  if (!supabaseUrl || !anonKey) {
    return { ok: false, message: 'Supabase Auth is not configured. Add SUPABASE_URL and SUPABASE_ANON_KEY in Vercel.' };
  }
  ...
  try {
    ...
    if (!response.ok) {
      return { ok: false, message: data.error_description || data.msg || data.message || 'Supabase rejected those credentials.' };
    }

    const userEmail = String(data.user?.email || email).toLowerCase();
    if (!isNoaAuthEmailAllowed(userEmail)) {
      return { ok: false, message: `${userEmail} is not authorised for NoA.` };
    }

    return { ok: true, email: userEmail };
  } catch {
    return { ok: false, message: 'NoA could not reach Supabase Auth. Try again shortly.' };
  }
}

function getPrimaryNoaAuthEmail() {
  return getNoaAuthAllowedEmails()[0] || BUDGET_OWNER_EMAIL;
}

function getNoaAuthAllowedEmails() {
  const configured = process.env.NOA_AUTH_EMAILS || process.env.NOA_AUTH_EMAIL || BUDGET_OWNER_EMAIL;
...
function isNoaAuthEmailAllowed(email) {
  const normalized = String(email || '').trim().toLowerCase();
  return Boolean(normalized) && getNoaAuthAllowedEmails().includes(normalized);
}
...
function getNoaAuthRateLimit(req) {
  cleanupNoaAuthAttempts();
  const key = getNoaAuthAttemptKey(req);
  const record = noaAuthAttempts.get(key);
  if (!record || record.count < NOA_AUTH_MAX_ATTEMPTS) return { blocked: false, retryAfterSeconds: 0 };
  const retryAfterSeconds = Math.max(1, Math.ceil((record.firstAttemptAt + NOA_AUTH_ATTEMPT_WINDOW_MS - Date.now()) / 1000));
  return { blocked: retryAfterSeconds > 0, retryAfterSeconds };
}

function recordNoaAuthFailure(req) {
  cleanupNoaAuthAttempts();
  const key = getNoaAuthAttemptKey(req);
  const existing = noaAuthAttempts.get(key);
  if (!existing || Date.now() - existing.firstAttemptAt > NOA_AUTH_ATTEMPT_WINDOW_MS) {
    noaAuthAttempts.set(key, { count: 1, firstAttemptAt: Date.now() });
    return;
  }
  existing.count += 1;
  noaAuthAttempts.set(key, existing);
}

function clearNoaAuthFailures(req) {
  noaAuthAttempts.delete(getNoaAuthAttemptKey(req));
}

function cleanupNoaAuthAttempts() {
  const now = Date.now();
  for (const [key, record] of noaAuthAttempts.entries()) {
    if (now - record.firstAttemptAt > NOA_AUTH_ATTEMPT_WINDOW_MS) noaAuthAttempts.delete(key);
  }
}

function getNoaAuthAttemptKey(req) {
...
function isTrustedRequestOrigin(req) {
  ...
  const allowedHosts = getAllowedAuthHosts(req);
  if (origin) {
    try {
...
function getAllowedAuthHosts(req) {
  const hosts = new Set([String(req.headers.host || '')].filter(Boolean));
  for (const value of [process.env.NOA_PUBLIC_APP_URL, process.env.VERCEL_PROJECT_PRODUCTION_URL, process.env.VERCEL_URL]) {
    if (!value) continue;
    try {
      hosts.add(new URL(value.startsWith('http') ? value : `https://${value}`).host);
    } catch {
      hosts.add(String(value).replace(/^https?:\/\//, '').replace(/\/.*$/, ''));
    }
  }
  hosts.add('no-a.vercel.app');
  return hosts;
}
...
function serializeCookie(name, value, req, options = {}) {
  ...
  return [
    ...
    `Max-Age=${options.maxAge ?? NOA_SESSION_TTL_SECONDS}`,
    `Path=${options.path || '/'}`,
...
async function askNoah(payload) {
  ...
  try {
    ...
    const notionJobs = env.NOTION_TOKEN && (notionDatabaseIds.tasksDataSourceId || notionDatabaseIds.tasksDatabaseId || notionDatabaseIds.jobsDataSourceId || notionDatabaseIds.jobsDatabaseId)
  ...
  try {
    ...
    if (response.status === 401) {
      return {
        ...
        ok: false,
        status: 401,
        message: 'OpenAI rejected OPENAI_API_KEY with 401. Replace the key in Vercel, make sure there are no extra spaces or quotes, then redeploy.'
      };
    }
...
function getNotionDatabaseIds(env) {
  return {
    clientsDatabaseId: normalizeNotionDatabaseId(env.NOTION_CLIENTS_DATABASE_ID || '5a836c85-e6ca-4fc5-b89e-b3b97b4bf38b'),
    clientsDataSourceId: normalizeNotionDataSourceId(env.NOTION_CLIENTS_DATA_SOURCE_ID || '9b4ead34-fcaf-4a27-a999-72248287878b'),
    tasksDatabaseId: normalizeNotionDatabaseId(env.NOTION_TASKS_DATABASE_ID || env.NOTION_DATABASE_ID || 'b5cdeb9c-0bcb-4c87-833b-ddeeee4ca956'),
    tasksDataSourceId: normalizeNotionDataSourceId(env.NOTION_TASKS_DATA_SOURCE_ID || '476fc915-819c-45e2-b990-8917290f675c'),
    pipelineViewId: normalizeNotionViewId(env.NOTION_PIPELINE_VIEW_ID || ''),
    tasksViewId: normalizeNotionViewId(env.NOTION_TASKS_VIEW_ID || ''),
    jobsDatabaseId: normalizeNotionDatabaseId(env.NOTION_JOBS_DATABASE_ID || '47b8cec5-c99a-4975-a2d6-ff1e990eb2b1'),
    jobsDataSourceId: normalizeNotionDataSourceId(env.NOTION_JOBS_DATA_SOURCE_ID || 'e9c28fc3-a5ab-44ff-b589-898a31b05e55')
  };
}
...
async function testNotion(token, databaseIds = {}) {
  ...
  const dataSourceId = databaseIds.tasksDataSourceId || databaseIds.jobsDataSourceId || databaseIds.clientsDataSourceId;
  const databaseId = databaseIds.tasksDatabaseId || databaseIds.jobsDatabaseId || databaseIds.clientsDatabaseId;

  try {
    const response = await fetch(dataSourceId
      ? `https://api.notion.com/v1/data_sources/${dataSourceId}/query`
      : databaseId
      ? `https://api.notion.com/v1/databases/${databaseId}/query`
      : 'https://api.notion.com/v1/users/me', {
      method: dataSourceId || databaseId ? 'POST' : 'GET',
      headers: {
        Authorization: `Bearer ${token}`,
        'content-type': 'application/json',
        'Notion-Version': dataSourceId ? '2025-09-03' : '2022-06-28'
      },
      ...(dataSourceId || databaseId ? { body: JSON.stringify({ page_size: 1 }) } : {})
    });
    ...
    return fromResponse(
      ...
      response,
      response.ok
        ? dataSourceId ? 'Notion token accepted and NoA data source reached.' : databaseId ? 'Notion token accepted and database reached.' : 'Notion token accepted.'
        : dataSourceId || databaseId ? 'Notion rejected the data request. Share the databases with the integration if this persists.' : 'Notion rejected the request.'
    );
  } catch (error) {
...
async function testHue(env) {
  const config = getHueConfig(env);
  if (!config.proxyUrl && (!config.bridgeUrl || (!config.username && !config.appKey))) {
    return missing('hue', 'Philips Hue', 'Add HUE_BRIDGE_URL and HUE_USERNAME, or configure HUE_PROXY_URL for remote access.');
  }

  const report = await getHueStatus(env);
  return {
    id: 'hue',
    name: 'Philips Hue',
...
async function getHueStatus(env = getEnv()) {
  return getHueReport(env, { includeLights: false });
}

async function getHueLights(env = getEnv()) {
  return getHueReport(env, { includeLights: true });
}

async function controlHue(payload = {}, env = getEnv()) {
  const targetType = payload.targetType === 'group' ? 'group' : 'light';
  const targetId = String(payload.targetId || payload.id || '').trim();
  if (!targetId) return { ok: false, message: 'Choose a Hue light or room first.' };

  const state = normaliseHueState(payload.state || {});
  if (!Object.keys(state).length) return { ok: false, message: 'No Hue state changes were provided.' };

  const config = getHueConfig(env);
  if (config.proxyUrl) {
    return callHueProxy(config, 'control', { targetType, targetId, state });
  }

  if (!config.bridgeUrl || !config.username) {
    return {
      ok: false,
      message: 'Hue direct control needs HUE_BRIDGE_URL and HUE_USERNAME. Use HUE_PROXY_URL if NoA is running from Vercel.'
    };
  }

  const route = targetType === 'group'
    ? `/api/${encodeURIComponent(config.username)}/groups/${encodeURIComponent(targetId)}/action`
    : `/api/${encodeURIComponent(config.username)}/lights/${encodeURIComponent(targetId)}/state`;
  const response = await hueFetch(config, route, {
    method: 'PUT',
...
async function getHueReport(env = getEnv(), options = {}) {
  const config = getHueConfig(env);
  const fetchedAt = new Date().toISOString();
  if (config.proxyUrl) {
    const result = await callHueProxy(config, options.includeLights ? 'lights' : 'status', {});
    return normaliseHueReport({ ...result, mode: 'proxy', fetchedAt });
  }

  if (!config.bridgeUrl || !config.username) {
    return normaliseHueReport({
      ok: false,
      mode: 'direct',
      fetchedAt,
      message: 'Hue is not configured. Add HUE_BRIDGE_URL and HUE_USERNAME in Vercel, then redeploy.',
      config: describeHueConfig(config)
    });
  }

  try {
    const [configResponse, lightsResponse, groupsResponse] = await Promise.all([
      hueFetch(config, `/api/${encodeURIComponent(config.username)}/config`),
      hueFetch(config, `/api/${encodeURIComponent(config.username)}/lights`),
      hueFetch(config, `/api/${encodeURIComponent(config.username)}/groups`)
    ]);
    ...
    if (!configResponse.ok || !lightsResponse.ok || !groupsResponse.ok || error) {
      return normaliseHueReport({
        ...
        mode: 'direct',
        fetchedAt,
        message: error || 'Hue bridge rejected the status request.',
        config: describeHueConfig(config)
      });
    }
    ...
    return normaliseHueReport({
      ...
      mode: 'direct',
      fetchedAt,
      message: `Hue bridge reached with ${lights.length} light${lights.length === 1 ? '' : 's'}.`,
      bridge: {
      ...
      lights: options.includeLights ? lights : lights.slice(0, 8),
      groups,
      config: describeHueConfig(config)
    });
  } catch (error) {
    return normaliseHueReport({
      ...
      mode: 'direct',
      fetchedAt,
      message: hueConnectionMessage(error, config),
      config: describeHueConfig(config)
    });
  }
}

function getHueConfig(env = getEnv()) {
  const bridgeUrl = String(env.HUE_BRIDGE_URL || env.PHILIPS_HUE_BRIDGE_URL || '').trim().replace(/\/$/, '');
  return {
    bridgeUrl,
    username: String(env.HUE_USERNAME || env.HUE_USER || env.PHILIPS_HUE_USERNAME || '').trim(),
    appKey: String(env.HUE_APP_KEY || env.HUE_APPLICATION_KEY || '').trim(),
    allowSelfSigned: String(env.HUE_ALLOW_SELF_SIGNED || '').toLowerCase() === 'true',
    proxyUrl: String(env.HUE_PROXY_URL || '').trim(),
    proxySecret: String(env.HUE_PROXY_SECRET || '').trim()
  };
}

function describeHueConfig(config) {
  return {
    directConfigured: Boolean(config.bridgeUrl && config.username),
    proxyConfigured: Boolean(config.proxyUrl),
    bridgeUrl: config.bridgeUrl ? maskBridgeUrl(config.bridgeUrl) : ''
  };
}
...
async function callHueProxy(config, action, payload) {
  try {
    const response = await fetch(config.proxyUrl, {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
        ...(config.proxySecret ? { 'x-noa-hue-secret': config.proxySecret } : {})
      },
      body: JSON.stringify({
        source: 'noa',
        action,
    ...
    return {
      ...
      message: data.message || (response.ok ? 'Hue proxy responded.' : 'Hue proxy rejected the request.'),
      ...data
    };
  } catch (error) {
    return {
      ok: false,
      status: 'error',
      message: `Hue proxy could not be reached. ${error instanceof Error ? error.message : 'Unknown error.'}`
    };
  }
}

async function hueFetch(config, path, options = {}) {
  const baseUrl = config.bridgeUrl || '';
  if (config.allowSelfSigned && baseUrl.startsWith('https://')) {
    process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0';
  }
  return fetch(`${baseUrl}${path}`, options);
}

function normaliseHueReport(report) {
  const lights = Array.isArray(report.lights) ? report.lights : [];
  ...
  return {
    ...
    message: report.message || (report.ok ? 'Hue is connected.' : 'Hue needs attention.'),
    bridge: report.bridge || null,
    lights,
    groups,
    totals: {
      lights: lights.length,
      rooms: groups.filter((group) => group.type === 'Room').length || groups.length,
      activeLights
    },
...
function hueConnectionMessage(error, config) {
  const message = error instanceof Error ? error.message : 'Unknown error.';
  if (config.bridgeUrl.includes('192.168.') || config.bridgeUrl.includes('10.') || config.bridgeUrl.includes('172.')) {
    return `Hue bridge was not reachable from this runtime. If NoA is deployed on Vercel, use HUE_PROXY_URL or run this from the home network. ${message}`;
  }
  return `Hue bridge could not be reached. ${message}`;
}
...
async function getNotionJobs() {
  const env = getEnv();
  const notionDatabaseIds = getNotionDatabaseIds(env);
  if (!env.NOTION_TOKEN || (!notionDatabaseIds.tasksDataSourceId && !notionDatabaseIds.tasksDatabaseId && !notionDatabaseIds.jobsDataSourceId && !notionDatabaseIds.jobsDatabaseId)) {
...
async function getHubGaugePayload() {
  const fetchedAt = Date.now();
  const errors = [];
  let notionJobs = emptyNotionJobs('');
  let screensavers = { cycleSeconds: 12, items: [] };

  try {
    notionJobs = await getNotionJobs();
    for (const error of [notionJobs.mainJobsError, notionJobs.tasksError, notionJobs.upcomingJobsError]) {
      if (error) errors.push(error);
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : 'Could not load Noah job context.');
  }

  let spotify = buildFallbackSpotify(fetchedAt);
  try {
    spotify = await getSpotifyNowPlaying(fetchedAt);
  } catch (error) {
    errors.push(error instanceof Error ? error.message : 'Could not load Spotify now-playing state.');
    spotify = { ...spotify, status: 'error' };
  }

  try {
    screensavers = await getHubGaugeScreensavers();
  } catch (error) {
    errors.push(error instanceof Error ? error.message : 'Could not load HubGauge screensavers.');
  }

  const jobs = buildHubGaugeJobs(notionJobs);

  return {
    status: errors.length ? 'partial' : 'online',
    deviceName: 'HubGauge',
    source: spotify.configured ? 'noah+notion+spotify' : 'noah+notion',
    mode: 'simulator',
    lastUpdated: new Date(fetchedAt).toLocaleTimeString('en-AU', {
      hour: 'numeric',
      minute: '2-digit',
    ...
    noah: {
      tone: 'calm',
      signal: jobs.next ? `Next up: ${jobs.next}` : 'Noah is ready for the road.',
      stale: false
    },
    errors
  };
...
async function getHubGaugeScreensavers() {
  const env = getEnv();
  const context = await getBudgetContext(env);
  if (!context?.ok) return { cycleSeconds: 12, items: [] };

  const personalisation = await fetchGroceryListPersonalisation(context);
  const enabledItems = personalisation.screensavers
    .filter((item) => item.enabled !== false && item.image)
    .slice(0, 8)
    .map((item, index) => ({
      id: item.id || `screensaver-${index + 1}`,
      name: item.name || `Screensaver ${index + 1}`
    }));

  return {
    cycleSeconds: clampNumber(personalisation.cycleSeconds, 5, 120, 12),
    items: enabledItems
  };
}

async function getHubGaugeScreensaverImage(id) {
  const env = getEnv();
  const context = await getBudgetContext(env);
  ...
  const personalisation = await fetchGroceryListPersonalisation(context);
  const item = personalisation.screensavers.find((entry) => entry.enabled !== false && entry.id === requestedId);
  if (!item?.image) return null;

  const image = String(item.image);
  const dataUrlMatch = image.match(/^data:(image\/(?:jpeg|jpg|png|webp));base64,(.+)$/i);
  if (dataUrlMatch) {
    const contentType = dataUrlMatch[1].toLowerCase() === 'image/jpg' ? 'image/jpeg' : dataUrlMatch[1].toLowerCase();
    return {
      contentType,
...
function buildHubGaugeJobs(report) {
  const taskItems = [
    ...(Array.isArray(report.pipelineTasks) ? report.pipelineTasks : []),
    ...(Array.isArray(report.taskList) ? report.taskList : [])
  ].filter((item, index, items) => item?.id && items.findIndex((candidate) => candidate.id === item.id) === index);
  const upcomingJobs = Array.isArray(report.upcomingJobs) ? report.upcomingJobs : [];
  const today = brisbaneDateKey(new Date());
  const tomorrowDate = new Date();
  tomorrowDate.setDate(tomorrowDate.getDate() + 1);
  const tomorrow = brisbaneDateKey(tomorrowDate);
  ...
  const datedItems = [
    ...upcomingJobs.map((job) => ({
      ...
      client: job.client || '',
      date: job.jobDate || '',
      dueState: job.dueState || ''
    })),
    ...taskItems.map((task) => ({
      title: task.title || 'Untitled task',
      client: task.assignees?.map((person) => person.name).filter(Boolean).join(', ') || '',
      date: task.shootDate || task.dueDate || '',
      dueState: task.shootState || task.dueState || ''
    }))
  ...
  const next = futureItems[0] || datedItems.sort((a, b) => String(a.date).localeCompare(String(b.date)))[0];
  const dueSoonStates = new Set(['Overdue', 'Due today', 'Today', 'Tomorrow', 'Due soon']);

  return {
    active: taskItems.filter((task) => !task.complete && !task.archived).length || upcomingJobs.filter((job) => !job.archived).length,
    today: datedItems.filter((item) => item.date === today || item.dueState === 'Due today' || item.dueState === 'Today').length,
    tomorrow: datedItems.filter((item) => item.date === tomorrow || item.dueState === 'Tomorrow').length,
    dueSoon: datedItems.filter((item) => dueSoonStates.has(item.dueState)).length,
    next: next?.title || 'No job synced',
...
function buildFallbackSpotify(fetchedAt) {
  return {
    configured: false,
    isPlaying: false,
    status: 'unconfigured',
    ...
    artist: 'Add SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, and SPOTIFY_REFRESH_TOKEN',
    album: '',
    image: '',
    progressMs: 0,
    durationMs: 180000,
...
async function getSpotifyNowPlaying(fetchedAt) {
  const env = getEnv();
  if (!env.SPOTIFY_CLIENT_ID || !env.SPOTIFY_CLIENT_SECRET || !env.SPOTIFY_REFRESH_TOKEN) {
    return buildFallbackSpotify(fetchedAt);
  }

  if (spotifyBackoffUntil > Date.now()) {
    throw new Error(`Spotify asked NoA to slow down. Retrying after ${new Date(spotifyBackoffUntil).toLocaleTimeString('en-AU', { timeZone: 'Australia/Brisbane' })}.`);
  }
  ...
  if (playingResponse.status === 429) {
    setSpotifyBackoff(playingResponse);
    throw new Error('Spotify rate limited the now-playing request.');
  }

  if (playingResponse.status === 204) {
    return {
      ...buildFallbackSpotify(fetchedAt),
      configured: true,
      status: 'idle',
  ...
  if (!playingResponse.ok) {
    const text = await safeReadText(playingResponse);
    throw new Error(`Spotify now-playing failed with ${playingResponse.status}. ${summariseApiError(text)}`);
  }

  const data = await playingResponse.json();
  const item = data.item || {};
  const image = Array.isArray(item.album?.images) ? item.album.images[0]?.url || '' : '';

  return {
    configured: true,
    isPlaying: data.is_playing === true,
    status: data.is_playing === true ? 'playing' : 'paused',
    trackId: item.uri || item.id || '',
    ...
    durationMs: Number(item.duration_ms || 180000),
    serverFetchedAt: fetchedAt,
    device: null
  };
}
...
async function getSpotifyAccessToken(env) {
  if (spotifyAccessTokenCache.accessToken && spotifyAccessTokenCache.expiresAt > Date.now() + 60000) {
  ...
  if (tokenResponse.status === 429) {
    setSpotifyBackoff(tokenResponse);
    throw new Error('Spotify rate limited the token refresh request.');
  }

  if (!tokenResponse.ok) {
    const text = await safeReadText(tokenResponse);
    throw new Error(`Spotify token refresh failed with ${tokenResponse.status}. ${summariseApiError(text)}`);
  }
...
function setSpotifyBackoff(response) {
  const retryAfterSeconds = Number(response.headers.get('retry-after') || 15);
  spotifyBackoffUntil = Date.now() + Math.max(5, retryAfterSeconds) * 1000;
}
...
async function updateNotionTaskStatus(payload = {}) {
  ...
  try {
    const currentPage = await fetchNotionPage(env.NOTION_TOKEN, pageId);
    const statusProperty = currentPage?.properties?.Status;
    ...
    return {
      ...
      item: mapNotionUpcomingJob(page)
    };
  } catch (error) {
    return {
      ok: false,
...
function buildTaskProperties(schema, values, options = {}) {
  const properties = {};
  assignWhenPresent(properties, schema, values, options, 'title', assignTitle, ['Task Name', 'Task name', 'Name']);
  assignWhenPresent(properties, schema, values, options, 'jobId', assignRelation, ['Job']);
  assignWhenPresent(properties, schema, values, options, 'assignedTo', assignSelect, ['Assigned To', 'Assignee']);
  ...
  assignWhenPresent(properties, schema, values, options, 'dueDate', assignDate, ['Due Date', 'Due date']);
  assignWhenPresent(properties, schema, values, options, 'shootDate', assignDate, ['Shoot Date']);
  assignWhenPresent(properties, schema, values, options, 'effortLevel', assignSelect, ['Effort Level', 'Effort level']);
  assignWhenPresent(properties, schema, values, options, 'description', assignRichText, ['Description']);
  assignWhenPresent(properties, schema, values, options, 'notes', assignRichText, ['Notes']);
  assignWhenPresent(properties, schema, values, options, 'capturedBy', assignSelect, ['Captured By']);
  assignWhenPresent(properties, schema, values, options, 'attachments', assignAttachments, ['Files / Links', 'Attachments', 'Attachment', 'Files', 'Links']);
  return properties;
}

function buildJobProperties(schema, values, options = {}) {
  const properties = {};
  assignWhenPresent(properties, schema, values, options, 'title', assignTitle, ['Job Name', 'Job name', 'Name']);
  assignWhenPresent(properties, schema, values, options, 'clientId', assignRelation, ['Client', 'Clients']);
  assignWhenPresent(properties, schema, values, options, 'client', assignText, ['Client name', 'Customer']);
  assignWhenPresent(properties, schema, values, options, 'status', assignStatus, ['Status']);
  assignWhenPresent(properties, schema, values, options, 'jobDate', assignDate, ['Shoot Date', 'Job date', 'Job Date', 'Date', 'Scheduled date']);
  assignWhenPresent(properties, schema, values, options, 'dueDate', assignDate, ['Due Date', 'Due date']);
  assignWhenPresent(properties, schema, values, options, 'priority', assignSelect, ['Priority']);
  assignWhenPresent(properties, schema, values, options, 'location', assignText, ['Location', 'Venue']);
  assignWhenPresent(properties, schema, values, options, 'johnsCut', assignNumber, ["John's Cut", 'Johns Cut', 'Pay ($AUD)', 'Pay', 'AUD']);
  assignWhenPresent(properties, schema, values, options, 'payAud', assignNumber, ["John's Cut", 'Johns Cut', 'Pay ($AUD)', 'Pay', 'AUD']);
  assignWhenPresent(properties, schema, values, options, 'description', assignRichText, ['Description']);
  assignWhenPresent(properties, schema, values, options, 'notes', assignRichText, ['Notes', 'Details']);
  assignWhenPresent(properties, schema, values, options, 'attachments', assignAttachments, ['Files / Links', 'Attachments', 'Attachment', 'Files', 'Links']);
  return properties;
}
...
function assignRelation(properties, schema, candidates, value) {
  const name = findPropertyName(schema, candidates, ['relation']);
  const ids = Array.isArray(value) ? value : String(value || '').split(',');
  const relation = ids
    .map((id) => normalizeNotionResourceId(id))
    .filter((id) => id && id.length >= 32)
    .map((id) => ({ id }));
  if (name && relation.length) properties[name] = { relation };
}

function assignNumber(properties, schema, candidates, value) {
  const name = findPropertyName(schema, candidates, ['number']);
  const number = Number(value);
  if (name) properties[name] = { number: Number.isFinite(number) ? number : null };
}
...
async function getBudgetSummary() {
  ...
  try {
    const [income, expenses, debts, mortgages, mortgageExpenses, assets, savings, settings, groceryItems] = await Promise.all([
      ...
      fetchBudgetGroceryItems(context)
    ]);
    ...
    return {
      ...
      ...buildBudgetReport({ owner: context.owner, income, expenses, debts, mortgages, mortgageExpenses, assets, savings, settings, groceryItems }),
...
async function getPublicGroceryListSummary(options = {}) {
  const env = getEnv();
  const context = await getBudgetContext(env);
  const includePersonalisation = options.includePersonalisation === true;
  if (!context.ok) {
    return {
      ok: false,
      message: context.message || 'Could not load the grocery list.',
      fetchedAt: new Date().toISOString(),
      owner: context.owner || { email: BUDGET_OWNER_EMAIL, displayName: '', userId: '' },
      groceryItems: [],
      personalisation: normalizeGroceryListPersonalisation({})
    };
  }

  try {
    const groceryItems = await fetchBudgetGroceryItems(context);
    const personalisation = includePersonalisation
      ? await fetchGroceryListPersonalisation(context)
      : normalizeGroceryListPersonalisation({});
    ...
    return {
      ...
      groceryItems,
      personalisation,
      personalisationIncluded: includePersonalisation
    };
  } catch (error) {
    return {
      ok: false,
      message: error instanceof Error ? error.message : 'Could not load the grocery list.',
      fetchedAt: new Date().toISOString(),
      owner: context.owner,
      groceryItems: [],
      personalisation: normalizeGroceryListPersonalisation({})
...
async function getNoaPersonalisationSettings() {
  const env = getEnv();
  const context = await getBudgetContext(env);
  if (!context.ok) return context;
  ...
  try {
    return {
      ok: true,
      message: 'Personalisation settings loaded.',
      fetchedAt: new Date().toISOString(),
      personalisation: {
        groceryList: await fetchGroceryListPersonalisation(context)
      }
    };
  } catch (error) {
    return {
      ok: false,
      message: error instanceof Error ? error.message : 'Could not load personalisation settings.',
      fetchedAt: new Date().toISOString(),
      personalisation: {
        groceryList: normalizeGroceryListPersonalisation({})
      }
    };
  }
}

async function saveNoaPersonalisationSettings(req) {
  const env = getEnv();
  const context = await getBudgetContext(env);
  if (!context.ok) return context;

  const payload = await readBody(req);
  const source = payload?.groceryList || payload?.personalisation?.groceryList || payload;
  const groceryList = await prepareGroceryListPersonalisationForStorage(context, normalizeGroceryListPersonalisation(source));

  try {
    const result = await saveNoaAppSetting(context, NOA_GROCERY_LIST_PERSONALISATION_KEY, groceryList);
    return {
      ok: true,
      message: 'Personalisation settings saved.',
      personalisation: { groceryList },
      row: result
    };
  } catch (error) {
    return { ok: false, message: error instanceof Error ? error.message : 'Could not save personalisation settings.' };
  }
}
...
async function manageGroceryItem(req) {
  const env = getEnv();
  const context = await getBudgetContext(env);
  if (!context.ok) return context;
  ...
  if (!['create', 'update', 'delete'].includes(action)) return { ok: false, message: 'Unknown grocery action.' };
  if (action !== 'create' && !id) return { ok: false, message: 'No grocery item id was provided.' };

  try {
    if (action === 'delete') {
      await supabaseRequest(context, `noa_grocery_items?id=eq.${encodeURIComponent(id)}&household_owner_user_id=eq.${encodeURIComponent(context.owner.userId)}`, { method: 'DELETE' });
      return { ok: true, message: 'Grocery item removed.' };
    }

    const row = normalizeGroceryWriteRow(values, context.owner);
    const result = action === 'create'
      ? await supabaseRequest(context, 'noa_grocery_items', { method: 'POST', body: JSON.stringify(row), prefer: 'return=representation' })
      : await supabaseRequest(context, `noa_grocery_items?id=eq.${encodeURIComponent(id)}&household_owner_user_id=eq.${encodeURIComponent(context.owner.userId)}`, { method: 'PATCH', body: JSON.stringify(row), prefer: 'return=representation' });

    return {
      ok: true,
      message: action === 'create' ? 'Grocery item added.' : 'Grocery item updated.',
      item: mapGroceryItem(Array.isArray(result) ? result[0] : result)
    };
  } catch (error) {
    return { ok: false, message: error instanceof Error ? error.message : 'Could not update grocery list.' };
  }
}

async function saveBudgetSettings(req) {
  const env = getEnv();
  const context = await getBudgetContext(env);
  if (!context.ok) return context;
  ...
  try {
    ...
    const normalized = normalizeBudgetSettings(payload, current);
    ...
    return {
      ...
      settings: {
        ...
        categories: normalized.rawData.categories,
        catColors: normalized.rawData.catColors,
        fuelCalculator: normalized.rawData.fuelCalculator
      },
      row: Array.isArray(result) ? result[0] : result
    };
  } catch (error) {
    return { ok: false, message: error instanceof Error ? error.message : 'Could not save budget settings.' };
  }
}

async function saveBudgetProfile(req) {
  const env = getEnv();
  const context = await getBudgetContext(env);
  if (!context.ok) return context;
  ...
  if (!displayName) return { ok: false, message: 'Enter a display name first.' };

  try {
    const result = await supabaseRequest(context, `user_profiles?user_id=eq.${encodeURIComponent(context.owner.userId)}`, {
      method: 'PATCH',
    ...
    return { ok: false, message: error instanceof Error ? error.message : 'Could not save profile.' };
  }
}
...
async function fetchBudgetTable(context, table) {
  const select = BUDGET_TABLE_SELECTS[table] || '*';
  const rows = await supabaseRequest(context, `${table}?user_id=eq.${encodeURIComponent(context.owner.userId)}&select=${select}&order=updated_at.desc.nullslast`);
  return Array.isArray(rows) ? rows.map((row) => mapBudgetRow(table, row)) : [];
}

async function fetchBudgetSettings(context) {
  const rows = await supabaseRequest(context, `ledger_settings?user_id=eq.${encodeURIComponent(context.owner.userId)}&select=user_id,default_mode,raw_data,updated_at&limit=1`);
  return Array.isArray(rows) ? rows[0] || null : null;
}

async function fetchNoaAppSetting(context, key) {
  const rows = await supabaseRequest(context, `noa_app_settings?key=eq.${encodeURIComponent(key)}&select=key,value,updated_at&limit=1`);
  return Array.isArray(rows) ? rows[0] || null : null;
}

async function saveNoaAppSetting(context, key, value) {
  const result = await supabaseRequest(context, 'noa_app_settings?on_conflict=key', {
    method: 'POST',
    body: JSON.stringify({
      key,
      value,
...
async function fetchGroceryListPersonalisation(context) {
  const appSetting = await fetchNoaAppSetting(context, NOA_GROCERY_LIST_PERSONALISATION_KEY);
  if (appSetting?.value && typeof appSetting.value === 'object') {
    const personalisation = normalizeGroceryListPersonalisation(appSetting.value);
    return migrateGroceryListPersonalisationIfNeeded(context, personalisation);
  }

  const legacySettings = await fetchBudgetSettings(context);
  const personalisation = normalizeGroceryListPersonalisation(legacySettings?.raw_data?.personalisation?.groceryList || {});
  return migrateGroceryListPersonalisationIfNeeded(context, personalisation);
}

async function migrateGroceryListPersonalisationIfNeeded(context, personalisation) {
  if (!personalisation.screensavers.some((item) => isDataUrlImage(item.image))) return personalisation;
  const migrated = await prepareGroceryListPersonalisationForStorage(context, personalisation);
  await saveNoaAppSetting(context, NOA_GROCERY_LIST_PERSONALISATION_KEY, migrated);
  return migrated;
}

async function prepareGroceryListPersonalisationForStorage(context, personalisation) {
  const prepared = [];

  for (const item of personalisation.screensavers) {
    if (!isDataUrlImage(item.image)) {
      prepared.push(item);
      continue;
    }

    const stored = await uploadScreensaverDataUrl(context, item);
    prepared.push({
      ...item,
      image: stored.publicUrl,
      imagePath: stored.path,
      storageBucket: NOA_SCREENSAVER_BUCKET
    });
  }

  return {
    ...personalisation,
    screensavers: prepared
  };
}
...
async function uploadScreensaverDataUrl(context, item) {
  const match = String(item.image || '').match(/^data:(image\/(?:jpeg|jpg|png|webp));base64,(.+)$/i);
  if (!match) throw new Error('Screensaver image was not a supported data URL.');

  await ensureScreensaverBucket(context);

  const contentType = match[1].toLowerCase() === 'image/jpg' ? 'image/jpeg' : match[1].toLowerCase();
  const extension = contentType === 'image/png' ? 'png' : contentType === 'image/webp' ? 'webp' : 'jpg';
  const buffer = Buffer.from(match[2], 'base64');
  const safeId = String(item.id || crypto.randomUUID()).replace(/[^a-z0-9_-]/gi, '-').slice(0, 80);
  const contentHash = crypto.createHash('sha1').update(buffer).digest('hex').slice(0, 12);
  const path = `grocery-list/${safeId}-${contentHash}.${extension}`;

  const response = await fetch(`${context.baseUrl}/storage/v1/object/${NOA_SCREENSAVER_BUCKET}/${path}`, {
    method: 'POST',
    headers: {
  ...
  const text = await safeReadText(response);
  if (!response.ok) throw new Error(`Supabase Storage upload failed with ${response.status}. ${summariseApiError(text)}`);

  return {
    path,
    publicUrl: `${context.baseUrl}/storage/v1/object/public/${NOA_SCREENSAVER_BUCKET}/${encodeStoragePath(path)}`
  };
}

async function ensureScreensaverBucket(context) {
  const existing = await fetch(`${context.baseUrl}/storage/v1/bucket/${NOA_SCREENSAVER_BUCKET}`, {
    headers: {
      apikey: context.serviceRoleKey,
      Authorization: `Bearer ${context.serviceRoleKey}`
    }
  ...
  const existingText = await safeReadText(existing);
  const bucketIsMissing = existing.status === 404 || (existing.status === 400 && /bucket not found/i.test(existingText));
  if (!bucketIsMissing) {
    throw new Error(`Supabase Storage bucket check failed with ${existing.status}. ${summariseApiError(existingText)}`);
  }

  const response = await fetch(`${context.baseUrl}/storage/v1/bucket`, {
    method: 'POST',
    headers: {
      apikey: context.serviceRoleKey,
      Authorization: `Bearer ${context.serviceRoleKey}`,
    ...
    body: JSON.stringify({
      id: NOA_SCREENSAVER_BUCKET,
      name: NOA_SCREENSAVER_BUCKET,
      public: true,
  ...
  if (!response.ok && response.status !== 409) {
    throw new Error(`Supabase Storage bucket create failed with ${response.status}. ${summariseApiError(text)}`);
  }
}
...
async function fetchBudgetGroceryItems(context) {
  const rows = await supabaseRequest(context, `noa_grocery_items?household_owner_user_id=eq.${encodeURIComponent(context.owner.userId)}&select=${GROCERY_ITEM_SELECT}&order=completed.asc,created_at.desc.nullslast`);
  return Array.isArray(rows) ? rows.map(mapGroceryItem) : [];
}
...
function buildBudgetReport({ owner, income, expenses, debts, mortgages, mortgageExpenses, assets, savings, settings, groceryItems = [] }) {
...
function normalizeGroceryWriteRow(values, owner) {
  const now = new Date().toISOString();
  const completedProvided = Object.prototype.hasOwnProperty.call(values, 'completed');
  const completed = completedProvided ? normalizeBoolean(values.completed, false) : undefined;
  const row = {
    household_owner_user_id: owner.userId,
    updated_at: now
  };
  if (Object.prototype.hasOwnProperty.call(values, 'item')) row.item = String(values.item || '').trim().slice(0, 160);
  if (Object.prototype.hasOwnProperty.call(values, 'quantity')) row.quantity = String(values.quantity || '').trim().slice(0, 80);
  if (Object.prototype.hasOwnProperty.call(values, 'category')) row.category = String(values.category || 'General').trim().slice(0, 80) || 'General';
  if (Object.prototype.hasOwnProperty.call(values, 'addedBy')) row.added_by = String(values.addedBy || owner.displayName || owner.email).trim().slice(0, 120);
  if (Object.prototype.hasOwnProperty.call(values, 'added_by')) row.added_by = String(values.added_by || owner.displayName || owner.email).trim().slice(0, 120);
  if (completedProvided) {
    row.completed = completed;
    row.completed_at = completed ? now : null;
  }
  if (!row.item && Object.prototype.hasOwnProperty.call(values, 'item')) throw new Error('Grocery item name is required.');
  if (!row.added_by && Object.prototype.hasOwnProperty.call(values, 'item')) row.added_by = owner.displayName || owner.email;
  if (Object.prototype.hasOwnProperty.call(values, 'item')) row.added_by_user_id = owner.userId;
  return row;
}
...
function normalizeBudgetSettings(payload = {}, current = null) {
  const source = payload && typeof payload === 'object' ? payload : {};
  const currentRaw = current?.raw_data && typeof current.raw_data === 'object' ? current.raw_data : {};
  const requestedMode = String(source.defaultMode || source.default_mode || current?.default_mode || currentRaw.defaultMode || 'personal').toLowerCase();
  const defaultMode = ['personal', 'business'].includes(requestedMode) ? requestedMode : 'personal';

  const currentCategories = Array.isArray(currentRaw.categories) ? currentRaw.categories : [];
  const currentColors = Array.isArray(currentRaw.catColors) ? currentRaw.catColors : [];
  const categories = source.categories === undefined
    ? normalizeStringList(currentCategories, BUDGET_DEFAULT_CATEGORIES)
    : normalizeStringList(source.categories, BUDGET_DEFAULT_CATEGORIES);
  const catColors = source.catColors === undefined
    ? normalizeColorList(currentColors, categories.length)
    : normalizeColorList(source.catColors, categories.length);

  const currentFuel = currentRaw.fuelCalculator && typeof currentRaw.fuelCalculator === 'object' ? currentRaw.fuelCalculator : {};
  const requestedFuel = source.fuelCalculator && typeof source.fuelCalculator === 'object' ? source.fuelCalculator : {};
  const fuelCalculator = {
    efficiency: sanitizeNumberText(requestedFuel.efficiency ?? currentFuel.efficiency ?? '6.3'),
    dailyKm: sanitizeNumberText(requestedFuel.dailyKm ?? currentFuel.dailyKm ?? '40'),
    tankSize: sanitizeNumberText(requestedFuel.tankSize ?? currentFuel.tankSize ?? '60'),
    fuelPrice: sanitizeNumberText(requestedFuel.fuelPrice ?? currentFuel.fuelPrice ?? ''),
    includeInBudget: normalizeBoolean(requestedFuel.includeInBudget ?? currentFuel.includeInBudget, true)
  };
  const currentPersonalisation = currentRaw.personalisation && typeof currentRaw.personalisation === 'object' ? currentRaw.personalisation : {};
  const requestedPersonalisation = source.personalisation && typeof source.personalisation === 'object' ? source.personalisation : {};
  const groceryListPersonalisation = normalizeGroceryListPersonalisation(
    requestedPersonalisation.groceryList === undefined
      ? currentPersonalisation.groceryList || {}
      : requestedPersonalisation.groceryList
  );
  ...
  return {
    ...
    rawData: {
      ...
      personalisation: {
        ...
        ...requestedPersonalisation,
        groceryList: groceryListPersonalisation
      }
    }
  };
}

function normalizeGroceryListPersonalisation(value = {}) {
  const source = value && typeof value === 'object' ? value : {};
  const items = Array.isArray(source.screensavers) ? source.screensavers : [];

  return {
    sleepMinutes: clampNumber(source.sleepMinutes ?? source.sleep_minutes, 1, 60, 5),
    cycleSeconds: clampNumber(source.cycleSeconds ?? source.cycle_seconds, 5, 120, 12),
    screensavers: items
      .map((item, index) => {
        const image = String(item.image || item.src || '').trim();
        if (!image) return null;
        return {
          id: String(item.id || `screensaver-${index + 1}`),
          name: String(item.name || `Screensaver ${index + 1}`).trim() || `Screensaver ${index + 1}`,
          image,
          imagePath: String(item.imagePath || item.image_path || '').trim(),
...
function normalizeColorList(value, length) {
  const input = Array.isArray(value) ? value : [];
  return Array.from({ length: Math.max(0, length) }, (_, index) => {
    const color = String(input[index] || BUDGET_DEFAULT_CATEGORY_COLORS[index % BUDGET_DEFAULT_CATEGORY_COLORS.length] || '#7dd3fc').trim();
    return /^#[0-9a-f]{6}$/i.test(color) ? color : BUDGET_DEFAULT_CATEGORY_COLORS[index % BUDGET_DEFAULT_CATEGORY_COLORS.length];
  });
}
...
function emptyBudgetReport(message, owner) {
  return { ok: false, message, fetchedAt: new Date().toISOString(), owner, tables: emptyBudgetTables(), totals: emptyBudgetTotals(), mortgageSummary: emptyMortgageSummary(), emailSettings: normalizeBudgetEmailSettings({}), tenantEmailActivity: [], groceryItems: [], settings: null };
}
...
function buildNoahInstructions() {
  return [
    ...
    'When asked about clients or jobs, inspect notionJobs.clients and notionJobs.upcomingJobs. Mention client status, retainer, industry/content type, job date, task rollups, pay, and location when available.',
...
async function fetchNotionJobs(token, databaseIds) {
  const jobsResult = await queryNotionCollection(token, {
    dataSourceId: databaseIds.jobsDataSourceId,
    databaseId: databaseIds.jobsDatabaseId,
    missingMessage: 'Jobs data source id is missing.'
  });
  const tasksResult = await queryNotionCollection(token, {
    dataSourceId: databaseIds.tasksDataSourceId,
    databaseId: databaseIds.tasksDatabaseId,
    missingMessage: 'Tasks data source id is missing.'
  });
  const clientsResult = await queryNotionCollection(token, {
    dataSourceId: databaseIds.clientsDataSourceId,
    databaseId: databaseIds.clientsDatabaseId,
    missingMessage: ''
  });

  const clients = clientsResult.results.map(mapNotionClient).filter((client) => client.title);
  const clientById = Object.fromEntries(clients.map((client) => [client.id, client]));
  const upcomingJobs = jobsResult.results
    .map((page) => mapNotionUpcomingJob(page, clientById))
    .filter((job) => job.title && !job.archived)
    .sort(sortUpcomingJobs);
  const jobById = Object.fromEntries(upcomingJobs.map((job) => [job.id, job]));
  const allTaskRows = tasksResult.results
    .map((page) => mapNotionTask(page, jobById))
    .filter((task) => Boolean(task.title) && !task.archived);
  const tasks = allTaskRows
    .filter(isActiveTask)
    .sort(sortTasksByDueDateAndPriority);
  const calendarTasks = tasksResult.results
    .map((page) => mapNotionTask(page, jobById))
    .filter((task) => Boolean(task.title) && !task.archived && !task.complete && task.shootDate)
    .sort(sortTasksByDueDateAndPriority);
  const pipelineTasks = allTaskRows
  ...
  return {
    ...
    taskList: tasks,
    calendarTasks,
    upcomingJobs: upcomingJobs.slice(0, 100),
    fetchedAt: new Date().toISOString(),
    mainJobsError: jobsResult.error || '',
    tasksError: tasksResult.error || '',
    upcomingJobsError: [jobsResult.error, clientsResult.error].filter(Boolean).join(' ')
  };
}

async function queryNotionCollection(token, { dataSourceId, databaseId, missingMessage }) {
  if (dataSourceId) return queryNotionDataSource(token, dataSourceId);
  if (databaseId) return queryNotionDatabase(token, databaseId);
  return { ok: true, results: [], error: missingMessage || '' };
}
...
async function queryNotionDataSource(token, dataSourceId) {
  try {
    ...
    while (hasMore) {
      ...
      if (!response.ok) {
        ...
        return {
          ok: false,
          results,
          error: `Notion data source request failed with status ${response.status}. ${summariseApiError(body)}`
        };
      }
    ...
    return {
      ok: false,
      results: [],
      error: error instanceof Error ? error.message : 'Unknown Notion data source error.'
    };
  }
}
...
function mapNotionClient(page) {
  const properties = page.properties || {};
  const jobIds = readFirstRelationIds(properties, ['Jobs']);
  return {
    id: page.id,
    title: readFirstTitle(properties, ['Client Name', 'Client name', 'Name', 'Client']),
    status: readFirstSelect(properties, ['Status']),
    priority: readFirstSelect(properties, ['Priority']),
    retainer: readFirstSelect(properties, ['Retainer']),
    budget: readFirstNumber(properties, ['Budget', 'Monthly Budget']),
    industry: readFirstMultiSelect(properties, ['Industry']),
    contentTypes: readFirstMultiSelect(properties, ['Content Type', 'Content Types']),
    coverImages: readFirstAttachments(properties, ['Cover Image', 'Cover Images']),
    jobIds,
...
function mapNotionTask(page, jobById = {}) {
  const properties = page.properties || {};
  const status = readStatusLike(properties.Status);
  const dueDate = readFirstDate(properties, ['Due Date', 'Due date']);
  const shootDate = readFirstDate(properties, ['Shoot Date', 'Shoot date', 'Job date', 'Job Date']);
  const complete = Boolean(page.archived) || isCompletedNotionStatus(status);
  const jobIds = readFirstRelationIds(properties, ['Job']);
  const linkedJob = jobIds.map((id) => jobById[id]).find(Boolean) || null;
  const assignedTo = readFirstSelect(properties, ['Assigned To', 'Assignee']);
  return {
    id: page.id,
    title: readFirstTitle(properties, ['Task Name', 'Task name', 'Name']),
    jobId: jobIds[0] || '',
    jobTitle: linkedJob?.title || '',
    client: linkedJob?.client || '',
    status,
    priority: readSelect(properties.Priority),
    dueDate,
    dueState: complete ? 'Complete' : describeDueDate(dueDate),
    shootDate,
    shootState: complete ? 'Complete' : describeDueDate(shootDate),
    effortLevel: readFirstSelect(properties, ['Effort Level', 'Effort level']),
    effortSize: effortSize(readFirstSelect(properties, ['Effort Level', 'Effort level'])),
    taskTypes: [],
    assignedTo,
    assignees: assignedTo ? [{ id: assignedTo, name: assignedTo, avatarUrl: null }] : readPeople(properties.Assignee),
    capturedBy: readFirstSelect(properties, ['Captured By']),
    description: readRichText(properties.Description),
    notes: readRichText(properties.Notes),
    attachments: readFirstAttachments(properties, ['Files / Links', 'Attachments', 'Attachment', 'Files', 'Links']),
    url: page.url,
    archived: Boolean(page.archived),
    complete,
...
function mapNotionUpcomingJob(page, clientById = {}) {
  const properties = page.properties || {};
  const title = readFirstTitle(properties, ['Job Name', 'Job name', 'Name']);
  const jobDate = readFirstDate(properties, ['Shoot Date', 'Job date', 'Job Date', 'Date', 'Scheduled date']);
  const dueDate = readFirstDate(properties, ['Due Date', 'Due date']);
  const status = readStatusLike(properties.Status);
  const clientIds = readFirstRelationIds(properties, ['Client', 'Clients']);
  const linkedClient = clientIds.map((id) => clientById[id]).find(Boolean) || null;
  const contentTypes = linkedClient?.contentTypes || readFirstMultiSelect(properties, ['Deliverable types', 'Deliverable Types', 'Task type', 'Type']);

  return {
    id: page.id,
    title,
    clientId: clientIds[0] || '',
    client: linkedClient?.title || readFirstText(properties, ['Client name', 'Customer']),
    status,
    jobDate,
    dueDate,
    dueState: describeDueDate(jobDate),
    priority: readFirstSelect(properties, ['Priority']),
    deliverableTypes: contentTypes,
    location: readFirstText(properties, ['Location', 'Venue']),
    johnsCut: readFirstNumber(properties, ["John's Cut", 'Johns Cut', 'Pay ($AUD)', 'Pay', 'AUD']),
    payAud: readFirstNumber(properties, ["John's Cut", 'Johns Cut', 'Pay ($AUD)', 'Pay', 'AUD']),
    description: readFirstText(properties, ['Description']),
    notes: readFirstText(properties, ['Notes', 'Details']),
    attachments: readFirstAttachments(properties, ['Files / Links', 'Attachments', 'Attachment', 'Files', 'Links']),
    taskCount: readFirstRollupNumber(properties, ['Task Count']),
    openTasks: readFirstRollupNumber(properties, ['Open Tasks']),
    completedTasks: readFirstRollupNumber(properties, ['Completed Tasks']),
    url: page.url,
    archived: Boolean(page.archived),
    complete: Boolean(page.archived) || isCompletedNotionStatus(status),
    column: statusToColumn(status),
    clientStatus: linkedClient?.status || '',
    clientPriority: linkedClient?.priority || '',
    clientRetainer: linkedClient?.retainer || '',
    clientIndustry: linkedClient?.industry || [],
    clientContentTypes: linkedClient?.contentTypes || [],
    clientCoverImages: linkedClient?.coverImages || [],
    effortSize: '',
    taskTypes: [],
    assignees: [],
    shootDate: jobDate,
    shootState: describeDueDate(jobDate)
  };
}
...
function statusToColumn(status) {
  return {
    'Notes/Client Info': 'Notes/Client Info',
...
function isCompletedNotionStatus(value) {
  const status = normalizeNotionStatusName(value);
  return status === 'Posted / Done' || status === 'Ready To Post' || status === 'Archived';
}
...
function normalizeNotionDataSourceId(id) {
  return normalizeNotionResourceId(id);
}
...
function readStatusLike(property) {
...
function readFirstRelationIds(properties, names) {
  for (const name of names) {
    const value = readRelationIds(properties[name]);
    if (value.length) return value;
  }
  return [];
}

function readFirstNumber(properties, names) {
  for (const name of names) {
    const property = properties[name];
    if (property?.type === 'number') return readNumber(property);
  }
  return 0;
}

function readFirstRollupNumber(properties, names) {
  for (const name of names) {
    const property = properties[name];
    if (!property) continue;
    if (property.type === 'number') return readNumber(property);
    if (property.type === 'rollup') {
      ...
      if (Array.isArray(property.rollup?.array)) return property.rollup.array.length;
    }
  }
  return 0;
}
