...
async function getPublicGroceryListSummary() {
  const env = getEnv();
  const context = await getBudgetContext(env);
  if (!context.ok) {
    return {
  ...
  try {
    const [personalisation, groceryItems] = await Promise.all([
      fetchGroceryListPersonalisation(context),
      fetchBudgetGroceryItems(context)
    ]);
...
async function fetchBudgetGroceryItems(context) {
  const rows = await supabaseRequest(context, `noa_grocery_items?household_owner_user_id=eq.${encodeURIComponent(context.owner.userId)}&select=*&order=completed.asc,created_at.desc.nullslast`);
