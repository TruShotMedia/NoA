...
async function handleNoaAuth(req, res) {
  ...
  if (action === 'status') {
    const unlocked = verifyNoaSessionCookie(req);
    return {
      ok: true,
      unlocked,
      locked: !unlocked,
...
function setNoaSessionCookie(req, res) {
  ...
  res.setHeader('set-cookie', serializeCookie(NOA_SESSION_COOKIE, token, req, {
    httpOnly: true,
...
function clearNoaSessionCookie(req, res) {
  res.setHeader('set-cookie', serializeCookie(NOA_SESSION_COOKIE, '', req, {
    httpOnly: true,
    maxAge: 0,
    sameSite: 'Lax',
    path: '/'
