...
function assignStatus(properties, schema, candidates, value) {
  ...
  if (!name || !String(value || '').trim()) return;
  if (schema[name]?.type === 'select') properties[name] = { select: { name: String(value).trim() } };
  else properties[name] = { status: { name: String(value).trim() } };
}
...
async function sendBudgetTenantEmail(req) {
  ...
  try {
    ...
    return {
      ok: sent.every((item) => item.ok),
      message: sent.every((item) => item.ok) ? `Sent ${sent.length} tenant email(s).` : 'One or more tenant emails could not be sent.',
      previews,
      sent
    };
  } catch (error) {
...
function buildMortgageSummary(mortgages, expenses) {
  const mortgagesWithBills = mortgages.map((mortgage) => {
    ...
    return {
      ...
      weeklyTenantBill: tenantCount > 0 ? (weeklyRepayment + weeklyOffset) / tenantCount : 0,
...
function budgetAllowedFields(kind) {
  ...
  if (kind === 'mortgageExpenses') return [...common, 'mortgage_local_id', 'offset_to_tenants', 'tenant_bill_weekly', 'tenant_bill_monthly', 'tenant_bill_per_tenant_weekly', 'tenant_bill_per_tenant_monthly'];
...
function buildTenantEmailPreview({ tenant, settings, mortgageSummary, from }) {
  ...
  const weeklyBill = mortgage ? mortgage.weeklyTenantBill : 0;
  const subject = `${settings.subjectPrefix}: ${formatCurrencyText(weeklyBill)} due this week`;
  const lines = [
    `Hi ${tenant.name || 'there'},`,
    '',
    `Your weekly property bill is ${formatCurrencyText(weeklyBill)}.`,
    mortgage?.name ? `Property: ${mortgage.name}` : '',
    mortgage?.propertyAddress ? `Address: ${mortgage.propertyAddress}` : '',
    mortgage ? `Included this week: ${formatCurrencyText(mortgage.weeklyRepayment)} mortgage repayment split plus ${formatCurrencyText(mortgage.weeklyOffsetExpenses)} in tenant-offset expenses.` : '',
  ...
  const html = tenantEmailHtml({ tenant, mortgage, weeklyBill, settings });
...
function tenantEmailHtml({ tenant, mortgage, weeklyBill, settings }) {
  const rows = [
    ['Weekly bill', formatCurrencyText(weeklyBill)],
    ['Mortgage repayment split', formatCurrencyText(mortgage?.weeklyRepayment || 0)],
    ['Tenant-offset expenses', formatCurrencyText(mortgage?.weeklyOffsetExpenses || 0)],
    ['Tenants', String(mortgage?.tenantCount || 0)]
  ];
  return `<!doctype html>
<html>
  <body style="margin:0;background:#f6f8fb;font-family:Arial,sans-serif;color:#111827;">
    <div style="max-width:620px;margin:0 auto;padding:28px 18px;">
      <div style="background:#ffffff;border-radius:18px;padding:28px;border:1px solid #e5e7eb;">
        ...
        <p style="margin:0 0 22px;font-size:16px;line-height:1.55;">Hi ${escapeHtml(tenant.name || 'there')}, your weekly property bill is <strong>${formatCurrencyText(weeklyBill)}</strong>.</p>
...
function emptyBudgetReport(message, owner) {
  return { ok: false, message, fetchedAt: new Date().toISOString(), owner, tables: emptyBudgetTables(), totals: emptyBudgetTotals(), mortgageSummary: emptyMortgageSummary(), emailSettings: normalizeBudgetEmailSettings({}), settings: null };
}
...
function mapNotionTask(page) {
  ...
  return {
    ...
    complete: Boolean(page.archived) || complete,
