...
async function getHubGaugeScreensaverImage(id) {
  ...
  if (/^https?:\/\//i.test(image)) {
    ...
    if (!response.ok) throw new Error(`Screensaver image fetch failed with ${response.status}.`);
...
async function saveNoaPersonalisationSettings(req) {
  ...
  const groceryList = normalizeGroceryListPersonalisation(source);
...
async function fetchGroceryListPersonalisation(context) {
  ...
  if (appSetting?.value && typeof appSetting.value === 'object') {
    return normalizeGroceryListPersonalisation(appSetting.value);
  }

  const legacySettings = await fetchBudgetSettings(context);
  return normalizeGroceryListPersonalisation(legacySettings?.raw_data?.personalisation?.groceryList || {});
}
