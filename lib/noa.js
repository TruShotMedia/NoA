...
async function buildUpdateProperties(token, pageId, kind, values) {
  ...
  return {
    ok: true,
    properties: kind === 'job'
      ? buildJobProperties(page.properties || {}, values)
      : buildTaskProperties(page.properties || {}, values)
  };
}
...
function buildTaskProperties(schema, values) {
  const properties = {};
  assignTitle(properties, schema, ['Task name', 'Name'], values.title);
  assignStatus(properties, schema, ['Status'], values.status);
  assignSelect(properties, schema, ['Priority'], values.priority);
  assignDate(properties, schema, ['Due date'], values.dueDate);
  assignDate(properties, schema, ['Shoot Date', 'Shoot date', 'Job date', 'Job Date'], values.shootDate);
  assignCheckbox(properties, schema, ['Complete', 'Completed', 'Done', 'Task complete'], values.complete);
  assignSelect(properties, schema, ['Effort level'], values.effortLevel);
  assignMultiSelect(properties, schema, ['Task type'], values.taskTypes);
  assignRichText(properties, schema, ['Description'], values.description);
  assignAttachments(properties, schema, ['Attachments', 'Attachment', 'Files', 'Links'], values.attachments);
  return properties;
}

function buildJobProperties(schema, values) {
  const properties = {};
  assignTitle(properties, schema, ['Job name', 'Job Name', 'Name', 'Task name'], values.title);
  assignText(properties, schema, ['Client', 'Client name', 'Customer'], values.client);
  assignDate(properties, schema, ['Job date', 'Job Date', 'Due date', 'Date', 'Shoot date', 'Scheduled date'], values.jobDate);
  assignSelect(properties, schema, ['Priority'], values.priority);
  assignMultiSelect(properties, schema, ['Deliverable types', 'Deliverable Types', 'Task type', 'Type'], values.deliverableTypes);
  assignText(properties, schema, ['Location', 'Venue'], values.location);
  assignRichText(properties, schema, ['Notes', 'Description', 'Details'], values.notes);
  assignAttachments(properties, schema, ['Attachments', 'Attachment', 'Files', 'Links'], values.attachments);
  return properties;
}
...
function assignCheckbox(properties, schema, candidates, value) {
  const name = findNamedPropertyName(schema, candidates, ['checkbox']) || findCheckboxPropertyName(schema);
  if (!name) return;
  properties[name] = { checkbox: value === true || String(value || '').toLowerCase() === 'true' };
}

function findCheckboxPropertyName(schema) {
  for (const [name, property] of Object.entries(schema)) {
